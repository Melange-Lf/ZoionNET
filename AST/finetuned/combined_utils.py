...
def evaluate_metric(
    ...
    metric_type: str,          # "accuracy", "auc" or "ap" (list)
    device: torch.device = None,
    dict_batches: dict = None
) -> torch.Tensor:
    """
    Runs model over dataloader and returns the requested metric.

    Args:
        model: a PyTorch module returning logits of shape (N, ...) .
        dataloader: yields (inputs, targets), where
            - targets can be
              * a 1D LongTensor of class‑indices (for binary or multiclass),
              * or a 2D FloatTensor of one‑hot / multi‑hot vectors (for multi-label).
        model_type: 
            - "binary"     single‑label binary classification.
            - "multi"      multi‑label classification (multi‑hot vectors).
        metric_type:
            - "accuracy"   FM.accuracy
            - "auc"        FM.auroc
            - "ap"         FM.average_precision
        device: torch.device to run on. Defaults to GPU if available.

    Returns:
        scalar metric (on the same device).
    """
    if device is None:
    ...
    with torch.no_grad():
        for i, batch in enumerate(dataloader):
            ...
            if not dict_batches:
                ...
                x, y = batch[dict_batches["x"]], batch[dict_batches["y"]]


            if i % print_freq == 0:
                print(f"starting {i}/{length}")


            x = x.to(device)
            # ensure targets are floats for multi‑hot, or ints for label‑indices
            y = y.to(device)
            ...
            if model_type == "binary":
                ...
                # logits: (N, C)
                # assume multi‑label → use sigmoid
                probs = torch.sigmoid(logits)
            else:
                raise ValueError(f"Unsupported model_type={model_type!r}")

            # if user passed class‐indices for multi, convert to one‐hot
            if model_type == "multi" and y.dim() == 1:
    ...
    if "accuracy" in metric_type:
        if model_type == "binary":
            out.append(FM.accuracy(preds, targets.long(), task="binary").to(device))
        else:
            out.append(FM.accuracy(preds, targets.int(), task="multilabel", num_labels=preds.size(1)).to(device))

    elif "auc" in metric_type:
        if model_type == "binary":
            out.append(FM.auroc(preds, targets.long(), task="binary").to(device))
        else:
            out.append(FM.auroc(preds, targets.int(), task="multilabel", num_labels=preds.size(1)).to(device))

    elif  "ap" in metric_type or "average_precision" in metric_type:
        if model_type == "binary":
            out.append(FM.average_precision(preds, targets.long(), task="binary").to(device))
        else:
            out.append(FM.average_precision(preds, targets.int(), task="multilabel", num_labels=preds.size(1)).to(device))
...
def evaluate_metric_ds(
    ...
    """
    Runs model over dataset and returns the requested metric.

    Args:
        model: a PyTorch module returning logits of shape (N, ...) .
        dataset: yields (inputs, targets), where
            - targets can be
              * a 1D LongTensor of class‑indices (for binary or multiclass),
              * or a 2D FloatTensor of one‑hot / multi‑hot vectors (for multi-label).
        model_type: 
    ...
    with torch.no_grad():
        for i, batch in enumerate(dataset):
            ...
            if not dict_batches:
                ...
                x, y = batch[dict_batches["x"]].unsqueeze(0), batch[dict_batches["y"]]
...
class FbankDataset(Dataset):
    ...
    Takes json file with structure file -> data -> (list of wav, labels)
    where wav is the path to the audio file (doesn't have to be .wav) and labels is string with class label ids seperated with ',' (not ', ' no space)

    the label csv should have at least 2 columns: index (what number you want to assign the class) and id (same as the one used in the json)
        ...
        if label_csv:
            with open(label_csv, 'r') as f:
                ...
                for row in csv_reader:
                    self.index_dict[row['id']] = row['index']
...
def train_model(model, train_loader, val_loader, args, optim_state_dict=None):
